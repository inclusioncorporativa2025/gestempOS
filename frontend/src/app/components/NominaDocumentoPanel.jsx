import React, { useCallback, useEffect, useState } from 'react';
import {
  Button,
  Card,
  DatePicker,
  Popconfirm,
  Space,
  Table,
  Typography,
  Upload,
  message,
} from 'antd';
import {
  DeleteOutlined,
  DownloadOutlined,
  InboxOutlined,
} from '@ant-design/icons';
import dayjs from 'dayjs';
import {
  descargarNomina,
  eliminarNomina,
  listarNominas,
  subirNomina,
} from '../../features/nominas/nominasService';
import './NominaDocumentoPanel.css';

const { Text, Title } = Typography;
const { Dragger } = Upload;

const MESES = [
  'Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio',
  'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre',
];

const etiquetaPeriodo = (mes, anio) => `${MESES[mes - 1] || mes} ${anio}`;

const formatearFecha = (fecha) => (
  fecha && dayjs(fecha).isValid() ? dayjs(fecha).format('DD/MM/YYYY HH:mm') : '—'
);

const NominaDocumentoPanel = ({ idUsuario }) => {
  const [loading, setLoading] = useState(true);
  const [subiendo, setSubiendo] = useState(false);
  const [soportado, setSoportado] = useState(true);
  const [documentos, setDocumentos] = useState([]);
  const [periodo, setPeriodo] = useState(dayjs().subtract(1, 'month').startOf('month'));
  const [archivoPendiente, setArchivoPendiente] = useState(null);

  const cargar = useCallback(async () => {
    if (!idUsuario) return;
    setLoading(true);
    try {
      const data = await listarNominas({ idUsuario });
      setSoportado(data.soportado !== false);
      setDocumentos(data.documentos || []);
    } catch (error) {
      message.error(error.message || 'No se pudieron cargar las nóminas');
    } finally {
      setLoading(false);
    }
  }, [idUsuario]);

  useEffect(() => {
    cargar();
  }, [cargar]);

  const handleSubir = async () => {
    if (!archivoPendiente) {
      message.warning('Selecciona un PDF antes de subir');
      return;
    }

    setSubiendo(true);
    try {
      await subirNomina({
        idUsuario,
        periodoMes: periodo.month() + 1,
        periodoAnio: periodo.year(),
        archivo: archivoPendiente,
      });
      message.success('Nómina subida correctamente');
      setArchivoPendiente(null);
      await cargar();
    } catch (error) {
      message.error(error.message || 'No se pudo subir la nómina');
    } finally {
      setSubiendo(false);
    }
  };

  const handleDescargar = async (doc) => {
    try {
      await descargarNomina(doc.id_documento, doc.nombre_archivo);
    } catch (error) {
      message.error(error.message || 'No se pudo descargar la nómina');
    }
  };

  const handleEliminar = async (idDocumento) => {
    try {
      await eliminarNomina(idDocumento);
      message.success('Nómina eliminada');
      await cargar();
    } catch (error) {
      message.error(error.message || 'No se pudo eliminar la nómina');
    }
  };

  if (!soportado) {
    return (
      <Card className="nomina-doc-panel">
        <Text type="secondary">
          El módulo de nóminas no está disponible. Ejecute el script SQL
          {' '}
          <code>documentos_nomina.sql</code>
          {' '}
          en el servidor.
        </Text>
      </Card>
    );
  }

  const columnas = [
    {
      title: 'Periodo',
      key: 'periodo',
      render: (_, row) => etiquetaPeriodo(row.periodo_mes, row.periodo_anio),
    },
    {
      title: 'Archivo',
      dataIndex: 'nombre_archivo',
      key: 'nombre_archivo',
    },
    {
      title: 'Publicada',
      dataIndex: 'fecha_publicacion',
      key: 'fecha_publicacion',
      render: formatearFecha,
    },
    {
      title: 'Vista empleado',
      dataIndex: 'visto_en',
      key: 'visto_en',
      render: (valor) => (valor ? formatearFecha(valor) : 'Pendiente'),
    },
    {
      title: 'Acciones',
      key: 'acciones',
      render: (_, row) => (
        <Space>
          <Button
            type="link"
            icon={<DownloadOutlined />}
            onClick={() => handleDescargar(row)}
          >
            Descargar
          </Button>
          <Popconfirm
            title="¿Eliminar esta nómina?"
            description="Se borrará el PDF del servidor."
            onConfirm={() => handleEliminar(row.id_documento)}
            okText="Eliminar"
            cancelText="Cancelar"
            okButtonProps={{ danger: true }}
          >
            <Button type="link" danger icon={<DeleteOutlined />}>
              Eliminar
            </Button>
          </Popconfirm>
        </Space>
      ),
    },
  ];

  return (
    <div className="nomina-doc-panel">
      <Card className="nomina-doc-panel__card">
        <Title level={5} className="nomina-doc-panel__title">
          Subir nómina definitiva (PDF)
        </Title>
        <Text type="secondary" className="nomina-doc-panel__hint">
          Asigna el PDF al empleado y al mes correspondiente. Si ya existe una nómina
          para ese periodo, se sustituirá.
        </Text>

        <div className="nomina-doc-panel__upload-row">
          <div>
            <Text strong>Periodo de la nómina</Text>
            <DatePicker
              picker="month"
              value={periodo}
              onChange={(value) => value && setPeriodo(value)}
              format="MMMM YYYY"
              className="nomina-doc-panel__periodo"
            />
          </div>

          <Dragger
            accept=".pdf,application/pdf"
            maxCount={1}
            fileList={archivoPendiente ? [{
              uid: '-1',
              name: archivoPendiente.name,
              status: 'done',
            }] : []}
            beforeUpload={(file) => {
              setArchivoPendiente(file);
              return false;
            }}
            onRemove={() => {
              setArchivoPendiente(null);
            }}
            className="nomina-doc-panel__dragger"
          >
            <p className="ant-upload-drag-icon">
              <InboxOutlined />
            </p>
            <p className="ant-upload-text">Arrastra el PDF o haz clic para seleccionarlo</p>
            <p className="ant-upload-hint">Solo PDF, máximo 5 MB</p>
          </Dragger>
        </div>

        <Button
          type="primary"
          onClick={handleSubir}
          loading={subiendo}
          disabled={!archivoPendiente}
        >
          Subir nómina
        </Button>
      </Card>

      <Card
        title="Nóminas asignadas"
        className="nomina-doc-panel__card"
        loading={loading}
      >
        <Table
          columns={columnas}
          dataSource={documentos}
          rowKey={(row) => `${row.empresa_id}-${row.id_documento}`}
          pagination={{ pageSize: 8 }}
          locale={{ emptyText: 'Sin nóminas subidas para este empleado' }}
          scroll={{ x: 720 }}
        />
      </Card>
    </div>
  );
};

export default NominaDocumentoPanel;
