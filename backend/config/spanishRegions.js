/**
 * Comunidades autónomas (códigos Nager.Date / ISO 3166-2) y provincias españolas.
 */
...
/** Provincias con prefijo postal (2 dígitos) y CCAA asociada. */
const PROVINCIAS = [
...
const regionCodeByCpPrefix = Object.fromEntries(
  PROVINCIAS.map((p) => [p.cpPrefix, p.regionCode]),
);

const regionCodeByProvinceName = Object.fromEntries(
  PROVINCIAS.map((p) => [p.name.toLowerCase(), p.regionCode]),
);

const provinceByCpPrefix = Object.fromEntries(
  PROVINCIAS.map((p) => [p.cpPrefix, p.name]),
);

const resolveRegionCode = ({ codigoRegionFestivos, codigoPostal, provincia } = {}) => {
  if (codigoRegionFestivos) return codigoRegionFestivos;

  const cp = String(codigoPostal ?? '').trim();
  if (cp.length >= 2) {
    const prefix = cp.slice(0, 2);
    if (regionCodeByCpPrefix[prefix]) return regionCodeByCpPrefix[prefix];
  }

  const prov = String(provincia ?? '').trim().toLowerCase();
  if (prov && regionCodeByProvinceName[prov]) {
    return regionCodeByProvinceName[prov];
  }

  return null;
};

const isValidRegionCode = (code) =>
  COMUNIDADES_AUTONOMAS.some((r) => r.code === code);

module.exports = {
  COMUNIDADES_AUTONOMAS,
  PROVINCIAS,
  regionCodeByCpPrefix,
  regionCodeByProvinceName,
  provinceByCpPrefix,
  resolveRegionCode,
  isValidRegionCode,
};
