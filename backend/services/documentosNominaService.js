...
const { nominasSoportaDocumentos, nominasSoportaImportesDocumento } = require('../utils/nominasCompat');
const {
  buildRelativePath,
  guardarPdfNomina,
  leerPdfNomina,
...
const mapDocumento = (row) => {
  ...
  return {
    ...
    importe_bruto: data.importe_bruto != null ? Number(data.importe_bruto) : null,
...
const subirDocumento = async (
  ...
  const soportaImportes = await nominasSoportaImportesDocumento();
  const importeBruto = soportaImportes ? parseImporteOpcional(importes.importe_bruto) : null;
  const importeDeducciones = soportaImportes ? parseImporteOpcional(importes.importe_deducciones) : null;
  const importeLiquido = soportaImportes ? parseImporteOpcional(importes.importe_liquido) : null;
  ...
  return sequelize.transaction(async (transaction) => {
    ...
    if (soportaImportes) {
      datosDocumento.importe_bruto = importeBruto;
      datosDocumento.importe_deducciones = importeDeducciones;
      datosDocumento.importe_liquido = importeLiquido;
    }
